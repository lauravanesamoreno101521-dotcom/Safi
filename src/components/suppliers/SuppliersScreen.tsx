  ...
  return (
    <div className="flex-1 p-6 lg:p-8 flex flex-col gap-6 overflow-y-auto bg-[#f1fbff]">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-[#131d21]">Directorio de Proveedores</h2>
        ...
        <button
          ...
          className="px-4 py-2.5 bg-[#9f3023] hover:bg-[#881f14] text-white font-bold text-xs rounded-xl shadow-xs flex items-center gap-2 cursor-pointer transition-colors w-fit"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {MOCK_SUPPLIERS.map((sup) => (
          <div
            key={sup.id}
            className="bg-white rounded-2xl p-6 border border-[#dfbfba] shadow-xs flex flex-col justify-between gap-4"
          >
            <div>
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 rounded-xl bg-[#fff2f0] text-[#9f3023] flex items-center justify-center">
                    <Truck className="w-6 h-6" />
                  </div>
                  <div>
                    <h3 className="font-bold text-base text-[#131d21]">{sup.name}</h3>
              ...
              <div className="mt-4 space-y-2 border-t border-gray-100 pt-3">
                <div className="flex items-center gap-2 text-xs text-gray-600">
                  ...
                  <span className="font-medium text-[#131d21]">{sup.category}</span>
                </div>
                <div className="flex items-center gap-2 text-xs text-gray-600">
            ...
            <div className="pt-3 border-t border-gray-100 flex justify-end gap-2">
              <button
                ...
                className="px-3 py-1.5 bg-[#e4f0f4] hover:bg-[#dae1e3] text-[#131d21] rounded-lg text-xs font-bold transition-colors"
              >
                Llamar
              </button>
              <button
                onClick={() => alert(`Generando orden de compra hacia ${sup.name}...`)}
                className="px-3 py-1.5 bg-[#fff2f0] hover:bg-[#c04838] hover:text-white text-[#9f3023] rounded-lg text-xs font-bold transition-colors"
              >
                Pedir Stock
              </button>
            </div>
